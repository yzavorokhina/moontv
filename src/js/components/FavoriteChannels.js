
export default function FavoriteChannels() {


    return (
        <>
            <div className="favourite-channels-wrapper">
                <div className="favourite-channels-titles">
                    <div className="favourite-channels-title">
                        <h2>Избранные каналы</h2>
                    </div>
                    <div className="favourite-channels-show-all">
                        <a href="#" className="arrowed">
                            Показать все
                            {/* <!-- License: MIT. Made by Shopify: https://github.com/Shopify/polaris --> */}
                            <svg className="show-all-arrow" width="24px" height="24px" viewBox="0 0 24 24"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M17.707 9.293l-5-5a.999.999 0 10-1.414 1.414L14.586 9H3a1 1 0 100 2h11.586l-3.293 3.293a.999.999 0 101.414 1.414l5-5a.999.999 0 000-1.414z"
                                    fill="#FF6F3A" />
                            </svg>
                        </a>
                    </div>
                </div>
                <div className="favourite-channels-blocks">
                    <div className="favorite-category">
                        <h4>Обучение</h4>
                    </div>
                    <div className="favorite-channels">
                        <ul>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-1-1.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">WeDontNeedNo</h6>
                                        <a href="#" className="category-name">English for astronauts</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-1-2.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">Rosenthal</h6>
                                        <a href="#" className="category-name">Speak and write correctly</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-1-3.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">S'il vous plait</h6>
                                        <a href="#" className="category-name">Etiquette in space</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div className="favorite-category">
                        <h4>Разработка</h4>
                    </div>
                    <div className="favorite-channels">
                        <ul>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-2-1.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">satriano</h6>
                                        <a href="#" className="category-name">Software and game development</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-2-2.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">LikeAPro</h6>
                                        <a href="#" className="category-name">Software and game development</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-2-3.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">VioletStar</h6>
                                        <a href="#" className="category-name">Software and game development</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div className="favorite-category">
                        <h4>Общение</h4>
                    </div>
                    <div className="favorite-channels">
                        <ul>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-3-1.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">LuckyKhajiit</h6>
                                        <a href="#" className="category-name">Skyrim Chatting</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-3-2.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">chatgpt_24_7</h6>
                                        <a href="#" className="category-name">Just Chatting</a>
                                    </div>
                                </div>
                            </li>
                            <li>
                                <div className="favorite-channel">
                                    <div className="favorite-channel-ava">
                                        <a href="">
                                            <img src="./images/favorite-channels-ava-3-3.svg" alt="personal account" />
                                        </a>
                                    </div>
                                    <div className="stream-live-indicator">
                                        <span>&#11044;</span>
                                    </div>
                                    <div className="favorite-channel-info">
                                        <h6 className="favorite-channel-title">Сhat Master</h6>
                                        <a href="#" className="category-name">Just Chatting</a>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div className="favorite-category">
                        <h4>Другое</h4>
                    </div>
                </div>
            </div>
        </>
    );
}